...
import { MOCK_KIRO_USAGE, emailToMockUserId, isMockUsageEnabled } from '@/lib/mock/kiro-usage';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (isMockUsageEnabled()) {
      const empMap = await getOrgProvider().getByEmails(MOCK_KIRO_USAGE.map((r) => r.email));
      const users: IdcUserStatus[] = MOCK_KIRO_USAGE.map((r) => {
        const emp = empMap.get(r.email);
        return {
