...
import { findMockUsageByUserId, isMockUsageEnabled } from '@/lib/mock/kiro-usage';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (isMockUsageEnabled()) {
      const mock = findMockUsageByUserId(userid);
      if (!mock) {
        return NextResponse.json({ error: 'User not found in mock data' }, { status: 404 });
      }
      const empMap = await getOrgProvider().getByEmails([mock.email]);
      const emp = empMap.get(mock.email);
      const response: UserDetailResponse = {
        userid,
        displayName: maskText(emp?.displayName || mock.displayName),
        email: maskEmail(mock.email),
        organization: maskText(mock.email.split('@')[1] || ''),
        department: emp?.department,
        team: emp?.team,
        jobGrade: emp?.jobGrade,
        summary: {
          totalMessages: mock.totalMessages,
          totalConversations: mock.totalConversations,
          totalCredits: mock.totalCredits,
          totalOverageCredits: mock.totalOverageCredits,
          activeDays: mock.activeDays,
          firstActive: mock.firstActive,
          lastActive: mock.lastActive,
        },
        dailyActivity: mock.dailyActivity,
        clientBreakdown: mock.clientBreakdown,
      };
