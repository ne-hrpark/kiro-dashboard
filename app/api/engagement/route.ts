...
import { MOCK_KIRO_USAGE, classifyTier, isMockUsageEnabled } from '@/lib/mock/kiro-usage';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (isMockUsageEnabled()) {
      const mockRows = MOCK_KIRO_USAGE.map((r) => ({
        userid: r.email,
        total_messages: String(r.totalMessages),
        total_conversations: String(r.totalConversations),
      }));
      return NextResponse.json(buildEngagement(mockRows));
    }
    ...
    return NextResponse.json(buildEngagement(rows));
  } catch (err) {
...
function buildEngagement(rows: Array<Record<string, string>>): EngagementData {
    let power = 0;
    let active = 0;
    let light = 0;
    let idle = 0;
